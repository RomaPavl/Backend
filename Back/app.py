...
app.config["JWT_SECRET_KEY"] = 'a9a170e3e74744a7971c795ad440347b'
jwt = JWTManager(app)
...
class StatusField(enum.Enum):
    ...
    the_credit_is_still_valid = 'the_credit_is_still_valid'
...
def fill_credit_data(credit_data, credit=None):
    ...
    deadline = register_at + relativedelta(months=12)
    status = ''
...
def create_user():  # put application's code here
    try:
        ...
        for user in User.query.all():
            print(f"User: {user}, user_data: {user_data}")
            if user == user_data["email"]:
                return make_response("email already taken", 400)
        ...
        response = ({"Error": err.normalized_messages()})
        return response
...
def create_credit():  # put application's code here
    try:
        credit_data = CreateCredit().load(request.json)
        for credit in Credit.query.all():
            print(f"Credit: {credit}, credit_data: {credit_data}")
...
def credit_actions(credit_id):
    ...
    if request.method == 'GET':
        ...
        try:
            credit_data = UpdateCredit().load(request.json)
            for credit in Credit.query.all():
                print(f"User: {credit}, user_data: {credit_data}")
            new_creditor, error_message = fill_credit_data(credit_data, credit)
            if error_message:
                return error_message, 400
...
def create_creditor():  # put application's code here
    try:
        creditor_data = CreateCreditor().load(request.json)
        for creditor in Credit.query.all():
            print(f"Creditor: {creditor}, creditor_data: {creditor_data}")
            if creditor == creditor_data["name"]:
                return make_response("Creitor already taken", 400)
...
def creditor_actions(creditor_id):
    ...
    if request.method == 'GET':
        ...
        try:
            creditor_data = UpdateCreditor().load(request.json)
            for creditor in Credit.query.all():
                print(f"User: {creditor}, user_data: {creditor_data}")
            new_creditor, error_message = fill_credit_data(creditor_data, creditor)
            if error_message:
                return error_message, 400
...
def pay_credit():  # put application's code here
    ...
    if not error_message:
        ...
        user = User.query.filter_by(user_id=new_payment.user_id).first()
        if not credit or not user:
